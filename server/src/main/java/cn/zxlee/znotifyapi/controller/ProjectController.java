...
public class ProjectController {
    ...
    public Result<ProjectVO> getProject(@RequestHeader String token, @NotEmpty @PathVariable("project_id") String projectId){
        HashMap<String, String> params  = new HashMap<String, String>() {{
            put("token", token);
        }};
        return Result.success((ProjectVO) projectService.getProject(params, projectId));
    }

    @PutMapping("/project/{project_id}")
    @ApiOperation("更新一条项目")
    public Result updateProject(@RequestHeader String token, @NotEmpty @PathVariable("project_id") String projectId, @Validated @RequestBody ProjectBO bo){
        HashMap<String, String> params  = new HashMap<String, String>() {{
            put("token", token);
        }};
        int result = projectService.updateOne(params, projectId, bo);
