...
public class ProjectServiceImpl extends BaseServiceImpl implements IProjectService<ProjectVO, ProjectBO, ProjectPageBO> {
    ...
    public int updateOne(Map map, String id, ProjectBO bo) {
        ...
        ProjectPO checkProjectPO = projectMapper.listByName(tokenUtils.getUserIdByToken(map.get("token").toString()), bo.getProjectName());
        if (null != checkProjectPO && !checkProjectPO.getId().equals(id)) {
            throw new CommonException("此项目名称已存在");
        }
        ProjectPO newProjectPO = BeanConvertUtils.convertTo(bo, ProjectPO::new);
        return projectMapper.updateOne(id, newProjectPO);
    }

    @Override
    public ProjectVO getProject(Map map, String id) {
        ProjectPO projectPO = projectMapper.listByUserIdAndId(tokenUtils.getUserIdByToken(map.get("token").toString()), id);
        if (null == projectPO) {
            throw new CommonException("当前项目不存在");
        }
        return BeanConvertUtils.convertTo(projectPO, ProjectVO::new);
    }
